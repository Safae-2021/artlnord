...
<div class="card mt-5 ">
    <div class="card-header ">
        <div class="row">
      <h3 class="card-title col-6"> Group 
    ...
    <div class="card-body p-2 ">
      <table class="table table-sm table-striped " id="example1" >
        ...
        <tbody>
           ...
           $i=0;
       @endphp 
       @forelse ($selectGroup as $group)
          <tr>
            <td>{{++$i}}.</td>
           
            <td>{{$group->label}}</td>       
            <td>{{$group->training->label}}</td>       
            <td>{{$group->room->label}}</td>       
            <td>{{$group->teacher->name}}</td>       
            <td> 
               <a href="{{route('edit.group',$group->id)}}" > <i class="fas fa-edit " style="color:#0a58ca"></i></a>                   
               <a href="{{route('delete.group',$group->id)}}"> <i class="fas fa-trash ml-3" style="color:red"></i> </a>
               {{-- <a href="{{route('group.profile',$group->id)}}"> <i class="fas fa-arrow-right ml-3" style="color:green"></i> </a> --}}
            </td>           
        </tr>
       @empty
           
           <tr>
            <td colspan="6" class="text-center"> aucun Group  </td>
         </tr>
       @endforelse
    ...
    <div class="card-footer clearfix">
      ...
      {{$selectGroup ->links('application.helpers.pagination')}}
