...
<div class="container-fluid " >
  <div class="card card-outline card-primary col-9 mt-5 " style="margin-left: 20%">
    ...
    <div class="card-body">
      ...
      <form action="{{route('update.requested.document',$selectRequestedDocumentInfo[0]->id)}}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="row ">
          <div class="col-lg-12">
        <label for="label">{{__('document demandés ')}}</label>
        <div class="input-group mb-3">
          <input type="text" class="form-control" id="label"  placeholder="document demandés " name="label" value="{{$selectRequestedDocumentInfo[0]->label}}">
  ...
  <div class="text-danger">{{$errors->first('status')}}</div>
        ...
        <div class="row">
          ...
          <div class="col-3 mt-2 pb-3" style="margin-left: 24%">
            <a  href="{{route('requested.document.list')}}" class="btn btn-primary btn-block">Annuler</a>
          </div>
          <!-- /.col -->
        </div>
      </form>
